...
Bitboard m_bishop_attacks[64] [512]; // 256 K
Bitboard m_rook_attacks  [64][4096]; // 2048K
...
int pretty_magics() {
  ...
  for(square = 0; square < 64; square++)
    printf("  0x%llxULL,\n", (unsigned long long)find_magic(square, RBits[square], 0));
  printf("};\n\n");

  printf("const Bitboard BMagic[64] = {\n");
  for(square = 0; square < 64; square++)
    printf("  0x%llxULL,\n", (unsigned long long)find_magic(square, BBits[square], 1));
  printf("};\n\n");
...
void init_magics() {
  ...
  for (int sq = 0; sq < 64; sq++) {
    m_bishop_table[sq].mask = bmask(sq);
    m_bishop_table[sq].magic = find_magic(sq, BBits[sq], 1);
  }
  for (int sq = 0; sq < 64; sq++) {
    m_rook_table[sq].mask = rmask(sq);
    m_rook_table[sq].magic = find_magic(sq, RBits[sq], 0);
  }
  ...
  for (int sq = 0; sq < 64; sq++) {
    for (int occ_index = 0; occ_index < (1 << BBits[sq]); occ_index++) {
      occ = index_to_uint64(occ_index, BBits[sq], m_bishop_table[sq].mask);
      index = transform(occ, m_bishop_table[sq].magic, BBits[sq]);
      m_bishop_attacks[sq][index] = batt(sq, occ);
    }
  }

  for (int sq = 0; sq < 64; sq++) {
    for (int occ_index = 0; occ_index < (1 << RBits[sq]); occ_index++) {
      occ = index_to_uint64(occ_index, RBits[sq], m_rook_table[sq].mask);
      index = transform(occ, m_rook_table[sq].magic, RBits[sq]);
      m_rook_attacks[sq][index] = ratt(sq, occ);
    }
  }
...
Bitboard fast_bishop_attacks(Square sq, Bitboard occ) {
  ...
  occ >>= 64 - BBits[sq];
  return m_bishop_attacks[sq][occ];
}
